...
while True:
    ...
    if video_category in user_data["disliked_categories"]:
        ...
        if not alternative_videos.empty:
            # Рекомендовать случайное видео из альтернативных категорий
            ...
            # Рекомендация видео из непривлекательных категорий
    ...
    if user_input == "like":
        user_data["liked_videos"].append(video_info['video_id'])  # Сохранение оригинального ID
        user_data["favorite_category"] = video_info['category_id']  # Обновление любимой категории
    elif user_input == "dislike":
        # Добавляем категорию в disliked_categories
    ...
    # Сохранение пользовательских данных в JSON
    user_data["liked_videos"] = [video for video in user_data["liked_videos"]]
    with open('user_data.json', 'w') as f:
        json.dump({
            "liked_videos": [str(video) for video in user_data["liked_videos"]],
            "favorite_category": str(user_data["favorite_category"]),
            "disliked_categories": list(user_data["disliked_categories"]),  # Сохранение непривлекательных категорий
        }, f)
